...
struct locality {
  ...
  explicit locality(const locality_data &in_data) {
    ...
    fi_address_ = 0;
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("expl constructing"), iplocality((*this))));
  }
  ...
  locality() {
    ...
    fi_address_ = 0;
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("default construct"), iplocality((*this))));
  }

  locality(const locality &other)
      : data_(other.data_), fi_address_(other.fi_address_) {
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("copy construct"), iplocality((*this))));
  }

  locality(const locality &other, fi_addr_t addr)
      : data_(other.data_), fi_address_(addr) {
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("copy fi construct"), iplocality((*this))));
  }

  locality(locality &&other)
      : data_(std::move(other.data_)), fi_address_(other.fi_address_) {
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("move construct"), iplocality((*this))));
  }

  // provided to support sockets mode bootstrap
  explicit locality(const std::string &address, const std::string &portnum) {
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("explicit construct"), address, ":", portnum));
    //
    ...
    fi_address_ = 0;
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("string constructing"), iplocality((*this))));
  }

  // some condition marking this locality as valid
  explicit inline operator bool() const {
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("bool operator"), iplocality((*this))));
    return (ip_address() != 0);
  }

  inline bool valid() const {
    LF_DEB(loc_deb,
           trace(NS_DEBUG::str<>("valid operator"), iplocality((*this))));
    return (ip_address() != 0);
  }
  ...
  locality &operator=(const locality &other) {
    ...
    LF_DEB(loc_deb, trace(NS_DEBUG::str<>("copy operator"), iplocality(*this),
                          iplocality(other)));
    return *this;
  }

  bool operator==(const locality &other) {
    LF_DEB(loc_deb, trace(NS_DEBUG::str<>("equality operator"),
                          iplocality(*this), iplocality(other)));
    return std::memcmp(&data_, &other.data_, locality_defs::array_size) == 0;
  }

  bool less_than(const locality &other) {
    LF_DEB(loc_deb, trace(NS_DEBUG::str<>("less operator"), iplocality(*this),
                          iplocality(other)));
  ...
  const uint32_t &ip_address() const {
    ...
    return reinterpret_cast<const struct sockaddr_in *>(data_.data())
        ->sin_addr.s_addr;
  ...
  inline char *fabric_data_writable() {
...
private:
  friend bool operator==(locality const &lhs, locality const &rhs) {
    LF_DEB(loc_deb, trace(NS_DEBUG::str<>("equality friend"), iplocality(lhs),
                          iplocality(rhs)));
  ...
  friend bool operator<(locality const &lhs, locality const &rhs) {
    ...
    LF_DEB(loc_deb, trace(NS_DEBUG::str<>("less friend"), iplocality(lhs),
                          iplocality(rhs)));
