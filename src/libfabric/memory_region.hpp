...
struct region_provider {
  ...
  // register region
  static inline int fi_register_memory(provider_domain *pd, int device_id,
                                       const void *buf, size_t len,
                                       uint64_t access_flags, uint64_t offset,
                                       uint64_t request_key,
                                       struct fid_mr **mr) {
    [[maybe_unused]] auto scp = NS_MEMORY::mrn_deb.scope(
        __func__, NS_DEBUG::ptr(buf), NS_DEBUG::dec<>(len), device_id);
    //
    struct iovec addresses = {/*.iov_base = */ const_cast<void *>(buf),
                              /*.iov_len = */ len};
    fi_mr_attr attr = {
        /*.mr_iov         = */ {&addresses},
        /*.iov_count      = */ 1,
    ...
    if (device_id >= 0) {
      ...
      LF_DEB(NS_MEMORY::mrn_deb, trace(NS_DEBUG::str<>("CUDA"), "set device id",
                                       device_id, handle));
    ...
    if (ret) {
      throw NS_LIBFABRIC::fabric_error(int(ret), "register_memory");
    }
    return ret;
  }
  ...
  static inline constexpr int access_flags() {
    return FI_READ | FI_WRITE | FI_RECV |
...
struct memory_handle {
  ...
  memory_handle()
      : address_{nullptr}, region_{nullptr}, size_{0}, used_space_{0} {}
  ...
  memory_handle(provider_region *region, unsigned char *addr,
      ...
      : address_{addr}, region_{region}, size_{uint32_t(size)}, used_space_{0} {
    //            LF_DEB(NS_MEMORY::mrn_deb,
  ...
  memory_handle(memory_handle &&other) noexcept
      : address_{other.address_},
        region_{std::exchange(other.region_, nullptr)}, size_{other.size_},
  ...
  // --------------------------------------------------------------------
  // Get the local descriptor of the memory region.
  inline void *get_local_key(void) const {
  ...
  int deregister(void) const {
    if (region_ /*&& !get_user_region()*/) {
      ...
      if (region_provider::unregister_memory(region_)) {
        ...
        LF_DEB(NS_MEMORY::mrn_deb,
               trace(NS_DEBUG::str<>("de-Registered region"), *this));
      }
      region_ = nullptr;
    }
    return 0;
  ...
  friend std::ostream &operator<<(std::ostream &os,
    ...
    os << "region "
       ...
       << " loc key "
       << NS_DEBUG::ptr(region.region_
                            ? region_provider::get_local_key(region.region_)
                            : nullptr)
       << " rem key "
       << NS_DEBUG::ptr(region.region_
                            ? region_provider::get_remote_key(region.region_)
                            : 0);
    ///// clang-format off
    ///// clang-format on
#endif
...
protected:
  ...
  // The hardware level handle to the region (as returned from libfabric
  // fi_mr_reg)
...
struct memory_segment : public memory_handle {
  ...
  // --------------------------------------------------------------------
  memory_segment(provider_region *region, unsigned char *address,
                 unsigned char *base_address, uint64_t size)
      : memory_handle(region, address, size), base_addr_(base_address) {}
  ...
  memory_segment(provider_domain *pd, const void *buffer, const uint64_t length,
                 bool bind_mr, void *ep, int device_id) {
    ...
    LF_DEB(NS_MEMORY::mrn_deb,
           trace(NS_DEBUG::str<>("memory_segment"), *this, device_id));

    int ret = region_provider::fi_register_memory(
        pd, device_id, buffer, length, region_provider::access_flags(), 0,
        key++, &(region_));
    if (!ret) {
      LF_DEB(NS_MEMORY::mrn_deb, trace(NS_DEBUG::str<>("Registered region"),
                                       "device", device_id, *this));
    }

    if (bind_mr) {
      ret = fi_mr_bind(region_, (struct fid *)ep, 0);
      if (ret) {
        throw NS_LIBFABRIC::fabric_error(int(ret), "fi_mr_bind");
      } else {
        LF_DEB(NS_MEMORY::mrn_deb,
               trace(NS_DEBUG::str<>("Bound region"), *this));
      }

      ret = fi_mr_enable(region_);
      if (ret) {
        throw NS_LIBFABRIC::fabric_error(int(ret), "fi_mr_enable");
      } else {
        LF_DEB(NS_MEMORY::mrn_deb,
               trace(NS_DEBUG::str<>("Enabled region"), *this));
      }
    }
  }
