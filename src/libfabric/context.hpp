...
public:
  ...
  using callback_queue =
      boost::lockfree::queue<detail::shared_request_state *,
...
public:
  ...
  static std::shared_ptr<controller_type>
  init_libfabric_controller(oomph::context_impl *ctx, MPI_Comm comm, int rank,
                            int size, int threads, bool debug = false);
...
public:
  ...
  region_type make_region(void *const ptr, std::size_t size, int device_id) {
    if (m_controller->get_mrbind()) {
      ...
      return libfabric::memory_segment(m_domain, ptr, size, true, endpoint,
                                       device_id);
    } else {
      return libfabric::memory_segment(m_domain, ptr, size, false, nullptr,
                                       device_id);
    }
  }
  ...
  bool cancel_recv(detail::shared_request_state *s) {
    ...
    bool ok = (fi_cancel(&(get_controller()->get_rx_endpoint().get_ep()->fid),
                         op_ctx) == 0);
...
register_memory<oomph::context_impl>(oomph::context_impl &c, void *const ptr,
                                     std::size_t size) {
...
template <>
inline oomph::libfabric::memory_segment
register_device_memory<context_impl>(context_impl &c, int device_id, void *ptr,
                                     std::size_t size) {
