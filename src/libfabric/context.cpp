...
context_impl::context_impl(MPI_Comm comm, bool thread_safe,
  ...
  LF_DEB(src_deb,
         debug(NS_DEBUG::str<>("Broadcast"), "rank", debug::dec<3>(rank),
               "context", debug::ptr(m_ctxt_tag)));

  // TODO fix the thread safety
  // problem: controller is a singleton and has problems when 2 contexts are
  // created in the following order: single threaded first, then multi-threaded
  // after int threads = thread_safe ? std::thread::hardware_concurrency() : 1;
  // int threads = std::thread::hardware_concurrency();
  int threads = boost::thread::physical_concurrency();
  m_controller =
      init_libfabric_controller(this, comm, rank, size, threads, debug);
...
const char *context_impl::get_transport_option(const std::string &opt) {
  if (opt == "name") {
    ...
    if (temp.size() > 31)
      throw std::runtime_error("Bad string option check, fix please");
...
context_impl::init_libfabric_controller(oomph::context_impl * /*ctx*/,
  ...
  if (!instance.get()) {
    LF_DEB(src_deb, debug(NS_DEBUG::str<>("New Controller"), "rank",
                          debug::dec<3>(rank), "size", debug::dec<3>(size),
                          "threads", debug::dec<3>(threads)));
    instance.reset(new controller_type());
    if (debug)
      instance->enable_debug();
    instance->initialize(HAVE_LIBFABRIC_PROVIDER, rank == 0, size, threads,
                         comm);
  }
  return instance;
}
