...
class communicator_impl : public communicator_base<communicator_impl> {
  ...
  using callback_queue =
      boost::lockfree::queue<detail::request_state *,
...
public:
  ...
  communicator_impl(context_impl *ctxt)
      : communicator_base(ctxt), m_context(ctxt), m_send_cb_queue(128),
        m_recv_cb_queue(128), m_recv_cb_cancel(8) {
    LF_DEB(com_deb<9>,
           debug(NS_DEBUG::str<>("MPI_comm"), NS_DEBUG::ptr(mpi_comm())));
  ...
  inline std::uint64_t
  make_tag64(std::uint32_t tag, /*std::uint32_t rank, */ std::uintptr_t ctxt) {
  ...
  inline void execute_fi_function(Func F, const char *msg, Args &&...args) {
    ...
    while (!ok) {
      ...
      if (ret == 0) {
        ...
        throw NS_LIBFABRIC::fabric_error(int(ret), msg);
      }
    }
  }
  ...
  void send_tagged_region(region_type const &send_region, std::size_t size,
                          fi_addr_t dst_addr_, uint64_t tag_,
    ...
    execute_fi_function(fi_tsend, "fi_tsend", m_tx_endpoint.get_ep(),
                        send_region.get_address(), size,
                        send_region.get_local_key(), dst_addr_, tag_, ctxt);
  }

  // --------------------------------------------------------------------
  // this takes a pinned memory region and sends it using inject instead of send
  void inject_tagged_region(region_type const &send_region, std::size_t size,
                            fi_addr_t dst_addr_, uint64_t tag_) {
    [[maybe_unused]] auto scp = com_deb<9>.scope(NS_DEBUG::ptr(this), __func__);
    // clang-format on
    LF_DEB(com_deb<9>, debug(NS_DEBUG::str<>("inject tagged"), "->",
                             NS_DEBUG::dec<2>(dst_addr_), send_region, "tag",
                             tag_disp(tag_), "tx endpoint",
    ...
    execute_fi_function(
        fi_trecv, "fi_trecv", m_rx_endpoint.get_ep(), recv_region.get_address(),
  ...
  // --------------------------------------------------------------------
  send_request
  send(context_impl::heap_type::pointer const &ptr, std::size_t size,
    ...
    if (size != reg.get_size()) {
      LF_DEB(com_err, error(NS_DEBUG::str<>("send mismatch"), "size",
                            NS_DEBUG::hex<6>(size), "reg size",
    ...
    send_tagged_region(reg, size, fi_addr_t(dst), stag,
                       &(s->m_operation_context));
    return {std::move(s)};
  }

  recv_request
  recv(context_impl::heap_type::pointer &ptr, std::size_t size, rank_type src,
       oomph::tag_type tag,
       util::unique_function<void(rank_type, oomph::tag_type)> &&cb,
       std::size_t *scheduled) {
    [[maybe_unused]] auto scp = com_deb<9>.scope(NS_DEBUG::ptr(this), __func__);
    std::uint64_t stag =
        make_tag64(tag, /*src, */ this->m_context->get_context_tag());
...
#ifdef EXTRA_SIZE_CHECKS
    if (size != reg.get_size()) {
      LF_DEB(com_err, error(NS_DEBUG::str<>("recv mismatch"), "size",
                            NS_DEBUG::hex<6>(size), "reg size",
                            NS_DEBUG::hex<6>(reg.get_size())));
    }
#endif
    ...
    recv_tagged_region(reg, size, fi_addr_t(src), stag,
                       &(s->m_operation_context));
    return {std::move(s)};
  }
  ...
  bool cancel_recv(detail::request_state *s) {
    ...
    LF_DEB(com_deb<9>, debug(NS_DEBUG::str<>("Cancel"), "ok", ok, "op_ctx",
                             NS_DEBUG::ptr(op_ctx)));
    ...
    while (!found) {
      ...
      while (!found && m_recv_cb_cancel.pop(temp)) {
        if (temp == s) {
          ...
          found = true;
          LF_DEB(com_deb<9>, debug(NS_DEBUG::str<>("Cancel"), "succeeded",
