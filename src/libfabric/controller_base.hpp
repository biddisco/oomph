...
static const char *libfabric_progress_string() {
...
static const char *libfabric_endpoint_string() {
...
template <typename Handle> void fidclose(Handle fid, const char *msg) {
  LF_DEB(NS_DEBUG::cnb_deb, debug(debug::str<>("closing"), msg));
  int ret = fi_close(fid);
  if (ret == -FI_EBUSY) {
    throw NS_LIBFABRIC::fabric_error(ret, "fi_close EBUSY");
...
public:
  ...
  endpoint_wrapper(fid_ep *ep, fid_cq *rq, fid_cq *tq, const char *name)
      : ep_(ep), rq_(rq), tq_(tq), name_(name) {
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this), __func__, name_);
  }
...
using endpoint_context_pool =
    boost::lockfree::queue<endpoint_wrapper,
...
struct stack_endpoint {
  ...
  ~stack_endpoint() {
    ...
    LF_DEB(NS_DEBUG::cnb_deb,
           trace(debug::str<>("Scalable Ep"), "used push", "ep",
                 NS_DEBUG::ptr(get_ep()), "tx cq", NS_DEBUG::ptr(get_tx_cq()),
                 "rx cq", NS_DEBUG::ptr(get_rx_cq())));
...
template <typename Derived> class controller_base {
public:
...
protected:
  ...
  static inline thread_local std::chrono::steady_clock::time_point
      send_poll_stamp;
  static inline thread_local std::chrono::steady_clock::time_point
      recv_poll_stamp;
...
public:
  ...
  void finvoke(const char *msg, const char *err, int ret) {
    ...
    if (ret)
      throw NS_LIBFABRIC::fabric_error(ret, err);
  }

public:
  // --------------------------------------------------------------------
  controller_base()
      : eps_(nullptr), tx_endpoints_(1), rx_endpoints_(1),
        display_fabric_info_(false), fabric_info_(nullptr), fabric_(nullptr),
        fabric_domain_(nullptr), ep_passive_(nullptr), av_(nullptr),
        tx_inject_size_(0), tx_attr_size_(0), rx_attr_size_(0),
        max_completions_per_poll_(1), msg_rendezvous_threshold_(0x4000),
        sends_posted_(0), recvs_posted_(0), sends_readied_(0),
        recvs_readied_(0), sends_complete(0), recvs_complete(0) {}
  ...
  endpoint_wrapper create_rx_endpoint(struct fid_domain *domain,
                                      struct fi_info *info, struct fid_av *av) {
  ...
  void initialize(std::string const &provider, bool rootnode, int size,
                  size_t threads, Args &&...args) {
    LF_DEB(NS_DEBUG::cnb_deb,
           eval([]() { std::cout.setf(std::ios::unitbuf); }));
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this), __func__);

    max_completions_per_poll_ = libfabric_completions_per_poll();
    LF_DEB(NS_DEBUG::cnb_err, debug(debug::str<>("Poll completions"),
                                    debug::dec<3>(max_completions_per_poll_)));

    uint32_t default_val = (threads == 1) ? 0x400 : 0x4000;
    msg_rendezvous_threshold_ = libfabric_rendezvous_threshold(default_val);
    LF_DEB(NS_DEBUG::cnb_err, debug(debug::str<>("Rendezvous threshold"),
                                    debug::hex<4>(msg_rendezvous_threshold_)));
    ...
    if (endpoint_type_ == endpoint_type::single) {
      ...
      auto tx_cq =
          bind_tx_queue_to_rx_endpoint(fabric_info_, eps_->ep_rx_.get_ep());
    ...
    defined(HAVE_LIBFABRIC_SHM) || defined(HAVE_LIBFABRIC_VERBS) ||            \
    defined(HAVE_LIBFABRIC_CXI) || defined(HAVE_LIBFABRIC_EFA)
    ...
    if (endpoint_type_ == endpoint_type::multiple) {
      ...
      auto ep_sx =
          new_endpoint_scalable(fabric_domain_, fabric_info_, true /*Tx*/,
                                threads, threads_allocated);

      LF_DEB(NS_DEBUG::cnb_deb,
             trace(debug::str<>("scalable endpoint ok"), "Contexts allocated",
                   debug::dec<4>(threads_allocated)));
      ...
      for (unsigned int i = 0; i < threads_allocated; i++) {
        [[maybe_unused]] auto scp = NS_DEBUG::cnb_deb.scope(
            NS_DEBUG::ptr(this), "scalable", debug::dec<4>(i));
        ...
        endpoint_wrapper tx(scalable_ep_tx, nullptr, scalable_cq_tx,
                            "tx scalable");
        LF_DEB(NS_DEBUG::cnb_deb,
    ...
    LF_DEB(NS_DEBUG::cnb_deb,
           debug(debug::str<>("setting 'here'"), iplocality(here_)));
    ...
    //            size_t threads_allocated = 0;
    //            auto   ep_sx = new_endpoint_scalable(fabric_domain_,
    //            fabric_info_, true /*Tx*/, threads,
    //                  threads_allocated);
    //            if (!ep_sx)
    //                throw NS_LIBFABRIC::fabric_error(FI_EOTHER, "fi_scalable
    //                endpoint creation failed");

    //            LF_DEB(NS_DEBUG::cnb_deb, trace(debug::str<>("scalable
    //            endpoint ok"),
    //                                         "Contexts allocated",
    //                                         debug::dec<4>(threads_allocated)));
    ...
    //            {
    //                [[maybe_unused]] auto scp =
    //                    NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this),
    //                    "scalable", debug::dec<4>(i));
    ...
    //                bind_queue_to_endpoint(scalable_ep_tx, scalable_cq_tx,
    //                FI_TRANSMIT, "tx scalable");

    //                enable_endpoint(scalable_ep_tx, "tx scalable");

    //                endpoint_wrapper tx(scalable_ep_tx, nullptr,
    //                scalable_cq_tx, "tx scalable"); LF_DEB(NS_DEBUG::cnb_deb,
    ...
    ////                    fi_rx_context(ep_sx, i, NULL, &scalable_ep_rx,
    /// NULL));

    ////                scalable_cq_rx =
    ////                    create_completion_queue(fabric_domain_,
    /// fabric_info_->rx_attr->size, "rx");

    ////                bind_queue_to_endpoint(scalable_ep_rx, scalable_cq_rx,
    /// FI_RECV, "rx scalable");

    ////                enable_endpoint(scalable_ep_rx, "rx scalable");

    ////                endpoint_wrapper rx(scalable_ep_rx, scalable_cq_rx,
    /// nullptr, "rx scalable"); /                LF_DEB(NS_DEBUG::cnb_deb, /
    /// trace(debug::str<>("Scalable Ep"), "initial rx push", "ep", /
    /// NS_DEBUG::ptr(rx.get_ep()), "tx cq", NS_DEBUG::ptr(rx.get_tx_cq()), "rx
    /// cq", /                        NS_DEBUG::ptr(rx.get_rx_cq()))); /
    ...
    //            eps_->ep_tx_ = endpoint_wrapper(ep_sx, nullptr, nullptr, "rx
    //            scalable");
  ...
  void open_fabric(std::string const &provider, int threads, bool rootnode) {
    ...
    LF_DEB(NS_DEBUG::cnb_deb, debug(debug::str<>("fabric provider"),
                                    fabric_hints_->fabric_attr->prov_name));
    ...
    if (display_fabric_info_ && fabric_info_) {
      const char *info_str = fi_tostr(fabric_info_, FI_TYPE_INFO);
      if (info_str) {
  ...
  int _set_check_domain_op_value(int op, T value, const char *info,
    ...
    if (ret == 0 && set) {
      ...
      LF_DEB(NS_DEBUG::cnb_deb, debug(debug::str<>("gni set ops val"), value,
                                      (ret == 0 ? "OK" : "FAIL")));
    }

    // Get the value (so we can check that the value we set is now returned)
    T new_value;
    ret = gni_domain_ops->get_val(&fabric_domain_->fid, (dom_ops_val_t)(op),
                                  &new_value);
    if constexpr (std::is_integral<T>::value) {
      LF_DEB(NS_DEBUG::cnb_err,
             debug(debug::str<>("gni op val"), (ret == 0 ? "OK" : "FAIL"), info,
                   debug::hex<8>(new_value)));
    } else {
      LF_DEB(NS_DEBUG::cnb_err,
             debug(debug::str<>("gni op val"), (ret == 0 ? "OK" : "FAIL"), info,
                   new_value));
  ...
  struct fid_ep *new_endpoint_active(struct fid_domain *domain,
    ...
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this), __func__);
    LF_DEB(NS_DEBUG::cnb_deb, debug(debug::str<>("Got info mode"),
  ...
  // --------------------------------------------------------------------
  struct fid_ep *new_endpoint_scalable(struct fid_domain *domain,
                                       struct fi_info *info, bool tx,
                                       size_t threads,
                                       size_t &threads_allocated) {
    // don't allow multiple threads to call endpoint create at the same time
    scoped_lock lock(controller_mutex_);
    ...
    if (!hints)
      throw NS_LIBFABRIC::fabric_error(0, "fi_dupinfo");
    ...
    if (ret)
      throw NS_LIBFABRIC::fabric_error(ret, "fi_getinfo");
    ...
    if (ret)
      throw NS_LIBFABRIC::fabric_error(ret, "fi_scalable_ep");
    LF_DEB(NS_DEBUG::cnb_deb,
  ...
  endpoint_wrapper &get_rx_endpoint() {
    static auto rx =
        NS_DEBUG::cnb_deb.make_timer(1, debug::str<>("get_rx_endpoint"));
    ...
    if (endpoint_type_ == endpoint_type::scalableTxRx) {
      if (eps_->tl_srx_.get_ep() == nullptr) {
        ...
        eps_->tl_srx_ =
            stack_endpoint(ep.get_ep(), ep.get_rx_cq(), ep.get_tx_cq(),
                           ep.get_name(), &rx_endpoints_);
        LF_DEB(NS_DEBUG::cnb_deb,
               trace(debug::str<>("Scalable Ep"), "pop rx", "ep",
  ...
  endpoint_wrapper &get_tx_endpoint() {
    if (endpoint_type_ == endpoint_type::threadlocalTx) {
      if (eps_->tl_tx_.get_ep() == nullptr) {
        [[maybe_unused]] auto scp = NS_DEBUG::cnb_deb.scope(
            NS_DEBUG::ptr(this), __func__, "threadlocal");
        ...
        LF_DEB(NS_DEBUG::cnb_deb,
               trace(debug::str<>("Threadlocal Ep"), "create Tx", "ep",
                     NS_DEBUG::ptr(ep_tx), "tx cq", NS_DEBUG::ptr(tx_cq),
                     "rx cq", NS_DEBUG::ptr(nullptr)));
        // for cleaning up at termination
        endpoint_wrapper ep(ep_tx, nullptr, tx_cq, "tx threadlocal");
        tx_endpoints_.push(ep);
        eps_->tl_tx_ =
            stack_endpoint(ep_tx, nullptr, tx_cq, "threadlocal", nullptr);
      }
      ...
      if (eps_->tl_stx_.get_ep() == nullptr) {
        ...
        if (!ok) {
          LF_DEB(NS_DEBUG::cnb_deb,
                 error(debug::str<>("Scalable Ep"), "pop tx", "ep",
                       NS_DEBUG::ptr(ep.get_ep()), "tx cq",
                       NS_DEBUG::ptr(ep.get_tx_cq()), "rx cq",
                       NS_DEBUG::ptr(ep.get_rx_cq())));
        ...
        LF_DEB(NS_DEBUG::cnb_deb,
               trace(debug::str<>("Scalable Ep"), "pop tx", "ep",
  ...
  // --------------------------------------------------------------------
  void bind_address_vector_to_endpoint(struct fid_ep *endpoint,
                                       struct fid_av *av) {
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this), __func__);

    LF_DEB(NS_DEBUG::cnb_deb,
           debug(debug::str<>("Binding AV"), "to", NS_DEBUG::ptr(endpoint)));
    int ret = fi_ep_bind(endpoint, &av->fid, 0);
    if (ret)
      throw NS_LIBFABRIC::fabric_error(ret, "bind address_vector");
  }

  // --------------------------------------------------------------------
  void bind_queue_to_endpoint(struct fid_ep *endpoint, struct fid_cq *&cq,
                              uint32_t cqtype, const char *type) {
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this), __func__, type);

    LF_DEB(NS_DEBUG::cnb_deb, debug(debug::str<>("Binding CQ"), "to",
                                    NS_DEBUG::ptr(endpoint), type));
    int ret = fi_ep_bind(endpoint, &cq->fid, cqtype);
    if (ret)
      throw NS_LIBFABRIC::fabric_error(ret, "bind cq");
  }

  // --------------------------------------------------------------------
  fid_cq *bind_tx_queue_to_rx_endpoint(struct fi_info *info,
                                       struct fid_ep *ep) {
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this), __func__);
    info->tx_attr->op_flags |= (FI_INJECT_COMPLETE | FI_COMPLETION);
    fid_cq *tx_cq =
        create_completion_queue(fabric_domain_, info->tx_attr->size, "tx->rx");
  ...
  void enable_endpoint(struct fid_ep *endpoint, const char *type) {
    ...
    if (ret)
      throw NS_LIBFABRIC::fabric_error(ret, "fi_enable");
  }
  ...
  locality get_endpoint_address(struct fid *id) {
    ...
    if (ret || (addrlen > locality_defs::array_size)) {
      std::string err = std::to_string(addrlen) + "=" +
                        std::to_string(locality_defs::array_size);
      NS_LIBFABRIC::fabric_error(ret,
                                 "fi_getname - error (address size ?) " + err);
    }
  ...
  // --------------------------------------------------------------------
  fid_pep *create_passive_endpoint(struct fid_fabric *fabric,
                                   struct fi_info *info) {
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnb_deb.scope(NS_DEBUG::ptr(this), __func__);
    ...
    if (ret) {
      throw NS_LIBFABRIC::fabric_error(ret, "Failed to create fi_passive_ep");
    }
    return ep;
  }
  ...
  // --------------------------------------------------------------------
  // returns true when all connections have been disconnected and none are
  // active
  ...
  void debug_print_av_vector(std::size_t N) {
    ...
    for (std::size_t i = 0; i < N; ++i) {
      int ret = fi_av_lookup(av_, fi_addr_t(i), addr.fabric_data_writable(),
                             &addrlen);
      addr.set_fi_address(fi_addr_t(i));
      if ((ret == 0) && (addrlen <= locality_defs::array_size)) {
        LF_DEB(NS_DEBUG::cnb_deb, debug(debug::str<>("address vector"),
                                        debug::dec<3>(i), iplocality(addr)));
      } else {
        LF_DEB(NS_DEBUG::cnb_err,
  ...
  fid_av *create_address_vector(struct fi_info *info, int N,
    ...
    if (ret)
      throw NS_LIBFABRIC::fabric_error(ret, "fi_av_open");
    return av;
  }

  // --------------------------------------------------------------------
  locality insert_address(const locality &address) {
    return insert_address(av_, address);
  }
  ...
  locality insert_address(fid_av *av, const locality &address) {
    ...
    LF_DEB(NS_DEBUG::cnb_deb, trace(debug::str<>("inserting AV"),
                                    iplocality(address), NS_DEBUG::ptr(av)));
    fi_addr_t fi_addr = 0xffffffff;
    int ret = fi_av_insert(av, address.fabric_data(), 1, &fi_addr, 0, nullptr);
    if (ret < 0) {
      throw NS_LIBFABRIC::fabric_error(ret, "fi_av_insert");
    } else if (ret == 0) {
