...
struct request_state : public util::enable_shared_from_this<request_state>,
  ...
  request_state(oomph::context_impl *ctxt, oomph::communicator_impl *comm,
                std::size_t *scheduled, rank_type rank, tag_type tag,
                cb_type &&cb)
  ...
  void create_self_ref() {
    ...
    // this is useful if we only keep a raw pointer around internally, which
    // still is supposed to keep the object alive
...
struct shared_request_state
  ...
  shared_request_state(oomph::context_impl *ctxt,
                       oomph::communicator_impl *comm,
                       std::atomic<std::size_t> *scheduled, rank_type rank,
                       tag_type tag, cb_type &&cb)
      : base{ctxt, comm, scheduled, rank, tag, std::move(cb)},
        m_operation_context{this} {
