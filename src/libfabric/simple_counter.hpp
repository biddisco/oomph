...
//
// This class is intended to provide a simple atomic counter that can be used as
// a performance counter, but that can be disabled at compile time so that it
...
template <typename T> struct simple_counter<T, true> {
  simple_counter() : value_{T()} {}

  simple_counter(const T &init) : value_{init} {}
...
template <typename T> struct simple_counter<T, false> {
  simple_counter() {}
