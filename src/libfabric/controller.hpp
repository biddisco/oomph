...
public:
  ...
  // --------------------------------------------------------------------
  void initialize_derived(std::string const &, bool, int, size_t,
                          MPI_Comm mpi_comm) {
...
#endif
    return hmem_flags | FI_MSG | FI_TAGGED | FI_RMA | FI_READ | FI_WRITE |
           FI_RECV | FI_SEND | FI_TRANSMIT | FI_REMOTE_READ | FI_REMOTE_WRITE;
  }

  // --------------------------------------------------------------------
  // we do not need to perform any special actions on init (to contact root
  // node)
  ...
  void MPI_exchange_localities(fid_av *av, MPI_Comm comm, int rank, int size) {
    [[maybe_unused]] auto scp =
        NS_DEBUG::cnt_deb<9>.scope(NS_DEBUG::ptr(this), __func__);
    ...
    if (rank > 0) {
      ...
      /*int err = */ MPI_Send(here_.fabric_data(), locality_defs::array_size,
                              MPI_CHAR,
                              0, // dst rank
                              0, // tag
                              comm);

      LF_DEB(NS_DEBUG::cnt_deb<9>, debug(debug::str<>("receiving all"), "size",
                                         locality_defs::array_size));

      MPI_Status status;
      /*err = */ MPI_Recv(localities.data(), size * locality_defs::array_size,
                          MPI_CHAR,
                          0, // src rank
                          0, // tag
                          comm, &status);
  ...
  int poll_send_queue(fid_cq *send_cq, void *user_data) {
    ...
    assert(max_completions_per_poll_ <= max_completions_array_limit_);
    {
      ...
      if (!bypass_tx_lock() && !lock.owns_lock()) {
        return -1;
      }
      ...
      if (ret == -FI_EAVAIL) {
        ...
        if ((e.flags & (FI_MSG | FI_SEND | FI_TAGGED)) != 0) {
          NS_DEBUG::cnt_err.error(
              "txcq Error FI_EAVAIL for "
              "FI_SEND with len",
              debug::hex<6>(e.len), "context", NS_DEBUG::ptr(e.op_context),
              "code", NS_DEBUG::dec<3>(e.err), "flags", debug::bin<16>(e.flags),
              "error",
              fi_cq_strerror(send_cq, e.prov_errno, e.err_data, (char *)e.buf,
                             e.len));
    ...
    if (ret > 0) {
      ...
      for (int i = 0; i < ret; ++i) {
        ++sends_complete;
        LF_DEB(NS_DEBUG::cnt_deb<9>,
               debug(debug::str<>("Completion"), i, debug::dec<2>(i),
                     "txcq flags",
  ...
  int poll_recv_queue(fid_cq *rx_cq, void *user_data) {
    ...
    assert(max_completions_per_poll_ <= max_completions_array_limit_);
    {
      ...
      // if we're not threadlocal and didn't get the lock,
      // then another thread is polling now, just exit
      if (!bypass_rx_lock() && !lock.owns_lock()) {
        return -1;
      }
      ...
      if (ret == -FI_EAVAIL) {
        ...
        if (e.err == FI_ECANCELED) {
          ...
          NS_DEBUG::cnt_err.error(
              debug::str<>("poll_recv_queue"), "error code",
              debug::dec<>(-e.err), "flags", debug::hex<6>(e.flags), "len",
              debug::hex<6>(e.len), "context", NS_DEBUG::ptr(e.op_context),
              "error msg",
              fi_cq_strerror(rx_cq, e.prov_errno, e.err_data, (char *)e.buf,
                             e.len));
        }
        operation_context *handler =
            reinterpret_cast<operation_context *>(e.op_context);
    ...
    if (ret > 0) {
      ...
      NS_DEBUG::cnt_err.error("unknown error in completion rxcq read");
    }
    return 0;
  }
