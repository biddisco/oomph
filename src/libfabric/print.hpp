...
template <int N, typename T> struct dec {
  constexpr dec(T const &v) : data_(v) {}
...
template <int N = 2, typename T> constexpr detail::dec<N, T> dec(T const &v) {
...
// ------------------------------------------------------------------
struct ptr {
  ptr(void const *v) : data_(v) {}
  ptr(std::uintptr_t const v) : data_(reinterpret_cast<void const *>(v)) {}
  void const *data_;
...
struct hex<N, T, typename std::enable_if<!std::is_pointer<T>::value>::type> {
  constexpr hex(T const &v) : data_(v) {}
  T const &data_;
...
template <int N = 4, typename T> constexpr detail::hex<N, T> hex(T const &v) {
...
template <int N = 8, typename T = int> struct bin {
  constexpr bin(T const &v) : data_(v) {}
  T const &data_;
...
template <int N = 8, typename T> constexpr detail::bin<N, T> bin(T const &v) {
...
template <int N = 20> struct str {
  constexpr str(char const *v) : data_(v) {}
...
struct mem_crc32 {
  mem_crc32(const void *a, std::size_t len, const char *txt)
      : addr_(reinterpret_cast<const std::uint8_t *>(a)), len_(len), txt_(txt) {
  }
...
void tuple_print(std::ostream &os, TupleType const &t,
...
inline std::ostream &operator<<(std::ostream &os,
                                current_thread_print_helper const &) {
...
inline std::ostream &operator<<(std::ostream &os,
                                current_time_print_helper const &) {
...
template <typename... Args> void warning(Args const &...args) {
  display("<WAR> ", args...);
}
...
template <typename... Args> void scope(Args const &...args) {
  display("<SCO> ", args...);
}
...
template <typename... Args> void timed(Args const &...args) {
  display("<TIM> ", args...);
}
} // namespace detail

template <typename... Args> struct scoped_var {
  ...
  ~scoped_var() {
    detail::display("<SCO> ", prefix_, debug::str<>("<< leave >>"),
                    buffered_msg);
  }
};

template <typename... Args> struct timed_var {
  ...
  timed_var(double const &delay, Args const &...args)
      : time_start_(std::chrono::steady_clock::now()), delay_(delay),
        message_(args...) {}

  bool elapsed(std::chrono::steady_clock::time_point const &now) const {
    double elapsed_ = std::chrono::duration_cast<std::chrono::duration<double>>(
                          now - time_start_)
                          .count();
...
///////////////////////////////////////////////////////////////////////////
template <bool enable> struct enable_print;
...
template <> struct enable_print<false> {
  ...
  template <typename... Args> constexpr void debug(Args const &...) const {}

  template <typename... Args> constexpr void warning(Args const &...) const {}

  template <typename... Args> constexpr void trace(Args const &...) const {}

  template <typename... Args> constexpr void error(Args const &...) const {}

  template <typename... Args> constexpr void timed(Args const &...) const {}
  ...
  template <typename... Args> constexpr bool scope(Args const &...) {
    return true;
  }
  ...
  template <typename T, typename V> constexpr void set(T &, V const &) {}
  ...
  template <typename Expr> constexpr bool eval(Expr const &) { return true; }
};
...
public:
  constexpr enable_print() : prefix_("") {}

  constexpr enable_print(const char *p) : prefix_(p) {}

  constexpr bool is_enabled() const { return true; }

  template <typename... Args> constexpr void debug(Args const &...args) const {
  ...
  template <typename... Args> constexpr void trace(Args const &...args) const {
    detail::trace(prefix_, args...);
  }

  template <typename... Args> constexpr void error(Args const &...args) const {
    detail::error(prefix_, args...);
  }

  template <typename... Args> scoped_var<Args...> scope(Args const &...args) {
  ...
  template <typename T, typename V> void set(T &var, V const &val) {
    var = val;
  }
  ...
  template <typename Expr> auto eval(Expr const &e) { return e(); }
};
